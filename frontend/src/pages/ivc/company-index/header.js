...
/**
 * Header component for the Company Index page.
 * Includes search, sorting, and CSV export functionalities.
 *
 * @component
 * @returns {JSX.Element} The rendered header section of the company index
 */
const Header = () => {
  // Array of available filter options
  const filterArray = ['Country', 'Product Vehicle', 'Tire'];
  // Array of available sorting options
  const sorted = ['Date added', 'A-Z, Top - Bottom', 'A-Z, Bottom - Top'];
  // State to track loading status during CSV export
  const [loading, setIsLoading] = useState(false);
  // State toggle to help re-trigger search effect
  const [checkSearchClick, setCheckSearchClick] = useState(false);

  /**
   * Handles search input changes by filtering dummy data.
   *
  ...
  /**
   * Sorts the company list alphabetically by company name.
   *
   * @returns {void}
   */
  const sortCompany =()=> {
  ...
  /**
   * Triggers download of current company list in CSV format.
   *
   * @returns {void}
   */
  const handleDownloadCsv = () => {
    ...
    // Simulate loading delay before resetting state
    setTimeout(()=>{
      setIsLoading(false);
    }, 2000);
  };
